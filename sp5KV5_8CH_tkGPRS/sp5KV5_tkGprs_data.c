...
StatBuffer_t pxFFStatBuffer;
	...
	FF_fread( &data_frame, sizeof(data_frame));
	FF_stat(&pxFFStatBuffer);
	...
	// Fecha y hora
	pos += snprintf_P( &gprs_printfBuff[pos],( sizeof(gprs_printfBuff) - pos ),PSTR( "%04d%02d%02d,"),data_frame.rtc.year,data_frame.rtc.month,data_frame.rtc.day );
	pos += snprintf_P( &gprs_printfBuff[pos],( sizeof(gprs_printfBuff) - pos ), PSTR("%02d%02d%02d"),data_frame.rtc.hour,data_frame.rtc.min, data_frame.rtc.sec );

	// Valores analogicos
	for ( channel = 0; channel < NRO_ANALOG_CHANNELS; channel++) {
		pos += snprintf_P( &gprs_printfBuff[pos],( sizeof(gprs_printfBuff) - pos ),PSTR(",%s=%.02f"),systemVars.aChName[channel],data_frame.an_mag_val[channel] );
	}
